...
void NextFit() {
  int currentBlock = 0;
  for (int i = 0; i < processCount; i++) {
    while (!memoryBlocks[currentBlock].isFree) {
      currentBlock++;
    }
    if (memoryBlocks[currentBlock].size >= process[i].size) {
      memoryBlocks[currentBlock].isFree = false;
      process[i].assigned = true;
      process[i].blockID = currentBlock;
      memoryBlocks[currentBlock].processID = process[i].ID;
      memoryBlocks[currentBlock].internalFragmentation = memoryBlocks[currentBlock].size - process[i].size;
      currentBlock++;
    }
  }
}
