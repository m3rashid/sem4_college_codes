...
int rowCheck(int magicSquare[][10], int row, int col) {
    ...
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) sum += magicSquare[i][j];
        if (i != 0 && sum != prevSum) return -1;
        prevSum = sum;
        sum = 0;
    }
    return prevSum;
...
int colCheck(int magicSquare[][10], int row, int col) {
    int i, j, sum = 0, prevSum = 0;
    for (int j = 0; j < col; j++) {
        for (int i = 0; i < row; i++) sum += magicSquare[i][j];
        if (j != 0 && sum != prevSum) return -1;
        prevSum = sum;
        sum = 0;
    }
    return prevSum;
...
int diagCheck(int magicSquare[][10], int row, int col) {
    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < col; j++) {
            if (i == j) diag1 += magicSquare[i][j];
            if (i + j == row - 1) diag2 += magicSquare[i][j];
        }
    }

    return diag1 != diag2 ? -1 : diag1;
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) scanf("%d", &magicSquare[i][j]);
    }
